...
Page({
  ...
  viewTaped: function (e) {
    var that = this;
    console.log(e);
    var decoration = that.data.needPatrolDecorations[e.currentTarget.id] || {};
    console.log("decoration");
    console.log(decoration);
    if (decoration) {
      wx.navigateTo({
        url: '../decorationPatrolDetail/decorationPatrolDetail?decoration=' + JSON.stringify(decoration),
      })
    }
    else {
      wx.showModal({
...
function getDecorationPatrolList(that) {
  var app = getApp();
  ...
  var data = {
    ...
    // isDone: 0,
    // orderType: orderType,
    serverURL: config.urls.getDecorationPatrolListUrl
  }
  wx.showLoading({
    title: '加载中...',
  })
  wx.request({
    ...
    success: res => {
      ...
      that.setData({
        needPatrolDecorations: data.needPatrolDecorations,
      })
      wx.setStorageSync("decorationDisposeWay", data.disposeWay);
      wx.setStorageSync("decorationPatrolItems", data.patrolItems);
      // if (decorationList) {
