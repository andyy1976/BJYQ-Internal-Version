...
Page({
  ...
  viewTaped: function (e) {
    var that = this;
    console.log(e);
    var equipmentTrouble = that.data.report[e.currentTarget.id] || {};
    console.log("equipmentTrouble");
    console.log(equipmentTrouble);
    if (equipmentTrouble) {
      wx.navigateTo({
        url: '../equipmentTroubleReportDetail/equipmentTroubleReportDetail?equipment=' + JSON.stringify(equipmentTrouble),
      })
    }
    else {
      wx.showModal({
        title: '提示',
        content: '获取设备故障详情失败,请稍后重试',
        showCancel: false
      })
    }
  },
